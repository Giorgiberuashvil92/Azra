...
export async function PATCH(request: Request, context: RouteContext) {
  ...
  return proxy(request, "PATCH", `/products/categories/${id}`);
}

export async function DELETE(request: Request, context: RouteContext) {
  const { id } = await context.params;
  return proxy(request, "DELETE", `/products/categories/${id}`);
}

async function proxy(request: Request, method: "PATCH" | "DELETE", path: string) {
  try {
